...
class NewsFragment : Fragment() {
    ...
    private val onScrollListener = object : RecyclerView.OnScrollListener() {
        ...
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            val hasReachedToEnd =
                topPosition + visibleItems >= sizeOfTheCurrentList // 스크롤이 끝에 다다랐는지 체크
            val shouldPaginate =
                !isLoading && !isLastPage && hasReachedToEnd && isScrolling // 다음 페이지를 불러와야하는지 체크
            if (shouldPaginate) {
                page++
    ...
    private fun setSearchView() {
        fragmentNewsBinding.svNews.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query: String?): Boolean {
                viewModel.getSearchedNews(query.toString(), country, page)
                viewSearchedNews()
                return false
            }
            ...
            override fun onQueryTextChange(newText: String?): Boolean {
                MainScope().launch {
                    delay(2000)
                    viewModel.getSearchedNews(newText.toString(), country, page)
                    viewSearchedNews()
                }
                return false
            }
